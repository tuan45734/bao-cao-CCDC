...
let currentUserRole = null;  // 'KV1'..'KV7' hoặc 'ADMIN'
let currentUserRoleName = '';

const ALL_KVS = ['KV1', 'KV2', 'KV3', 'KV4', 'KV5', 'KV6', 'KV7'];
...
function getKVsForRegion(region) {
    return REGION_KVS[region] || [];
}

function getRegionLabel(region) {
    return REGION_LABELS[region] || 'Tất cả miền';
}

function getKVLabel(kv) {
    return KV_LABELS[kv] || kv;
}
...
function syncKVOptions(kvSelect, visibleKVs, selectedKV) {
    const isKVVisible = (kv) => visibleKVs.includes(kv);

    for (const option of kvSelect.options) {
        if (option.value === 'all') {
            option.hidden = false;
        ...
        const allowed = isKVVisible(option.value);
        option.hidden = !allowed;
        option.disabled = false;
    }

    if (selectedKV !== 'all' && !isKVVisible(selectedKV)) {
        selectedKV = visibleKVs.length > 0 ? visibleKVs[0] : 'all';
    }
    ...
    if (selectedKV === 'all') {
        ...
        statsCards.innerHTML = `
            ...
            <div class="stat-card">
                <div class="stat-icon">🏠</div>
                <h4>Tổng số NPP</h4>
                <div class="number">${visibleKVs.reduce((total, kv) => total + (kvStats.get(kv)?.details.length || 0), 0)}</div>
...
function updateOverviewChart(kvStats, selectedKV, visibleKVs) {
    ...
    if (selectedKV === 'all') {
        ...
        chartTitle.innerHTML = `Chi tiết kệ theo NPP - ${getKVLabel(selectedKV)}`;
        const stat = kvStats.get(selectedKV) || createEmptyStat();
    ...
    if (selectedKV === 'all') {
        ...
        (visibleKVs.length > 0 ? visibleKVs : ['KV_EMPTY']).forEach(kv => {
            if (kv === 'KV_EMPTY') {
                ...
                uploadedLevel2.push(0);
                uploadedLevel3.push(0);
                notUploadedLevel1.push(0);
                notUploadedLevel2.push(0);
                notUploadedLevel3.push(0);
                return;
            }
            const stat = kvStats.get(kv);
...
function updateDataTable(kvStats, selectedKV, visibleKVs, searchTerm = '') {
    ...
    if (selectedKV === 'all') {
        ...
        if (displayKVs.length === 0) {
            const noDataRow = document.createElement('tr');
            noDataRow.innerHTML = `<td colspan="7" class="no-data">🔍 Miền ${getRegionLabel(currentRegion)} hiện chưa có dữ liệu</td>`;
            tableBody.appendChild(noDataRow);
        }
    } else {
        tableTitle.innerHTML = `Dữ liệu chi tiết - ${getKVLabel(selectedKV)}`;
        const stat = kvStats.get(selectedKV) || createEmptyStat();
        let filteredDetails = stat.details;
        if (searchTerm) {
            filteredDetails = stat.details.filter(d => d.npp.toLowerCase().includes(searchTerm.toLowerCase()));
